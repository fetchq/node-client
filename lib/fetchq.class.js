...
class Fetchq {
  constructor(settings = {}) {
    ...
    this.emitter = this.createEmitter(settings, this.logger);
  ...
  createPool(settings) {
    ...
    if (settings.connect) {
      ...
      config.connectionString = process.env.PGSTRING
        ...
        : `postgresql://${process.env.PGUSER || 'postgres'}:${process.env
            .PGPASSWORD || 'postgres'}@${process.env.PGHOST ||
            'localhost'}:${process.env.PGPORT || '5432'}/${process.env
            .PGDATABASE || 'postgres'}`;
    }
