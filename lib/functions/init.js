...
const createInit = (ctx) => async () => {
  try {
    ...
    // It is possible to skip the extension check if the db instance
    // was set up with plain SQL queries
    if (!ctx.settings.skipExtension) {
      try {
        await ctx.pool.query('CREATE EXTENSION IF NOT EXISTS "fetchq";')
      } catch (err) {
        // eslint-disable-next-line
        throw new Error('[FetchQ] failed to create the extension, if you installed FetchQ with plain SQL you should add `skipExtension: true` to your client settings.');
      }
    }
